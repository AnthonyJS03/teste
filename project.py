...
def gerar_nuvem_emprestimos(emprestimo_aluno, ano_selecionado, mes_selecionado):
    ...
    # Normaliza a frequência para garantir que livros com a mesma quantidade tenham o mesmo tamanho
    livros_freq_normalizado = {}
    max_frequencia = max(livros_freq.values())

    for livro, frequencia in livros_freq.items():
        # Normaliza para garantir que a mesma frequência tenha o mesmo peso
        livros_freq_normalizado[livro] = frequencia / max_frequencia

    # Geração da nuvem de palavras com base nas frequências normalizadas
    wordcloud = WordCloud(
        width=2000, height=800, max_words=30, background_color="white"
    ).generate_from_frequencies(livros_freq_normalizado)
...
dicionario = {"EM_DIA": "Em dia", "ATRASADO": "Em atraso"} # Colocar ATRASADO no lugar de EM_ATRASO
...
with col3:
    ...
    ano_selecionado = st.selectbox(
        "Selecione o ano", ano_opcoes, label_visibility="collapsed", placeholder="Selecione o ano"
    )
    st.write("")
...
with coluna2:
    if "Todos os meses" in mes_selecionado or not mes_selecionado:
        ...
        total_alunos_filtrado['Nome'] = total_alunos_filtrado['Nome'].apply(quebrar_linha)
        ...
        total_alunos_filtrado_mes_selecionado['Nome'] = total_alunos_filtrado_mes_selecionado['Nome'].apply(quebrar_linha)
